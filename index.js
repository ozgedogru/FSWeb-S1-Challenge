...
console.log(`Dizideki ilk fenomenin profil adi: ${fenomenler[0]["profile"]}`);

//(2) Dizideki üçüncü fenomenin (2. dizin) takipçi (followers) sayısı
console.log(
  "Dizideki ucuncu fenomenin takipci sayisi:",
...
console.log(`Yazim hatasi duzeltildi: ${fenomenler[6]["profile"]}`);
...
function indekseGoreFenomen(arr, indNumber) {
  return `${indNumber}. indekste bulunan fenomen: ${arr[indNumber]["profile"]}`;
}
...
function enFenomenler(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i]["followers"] > 100000000) {
      listFenomen.push(arr[i]["profile"]);
    }
  }
  return listFenomen;
}
